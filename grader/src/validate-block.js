...
import {generateMerkleRoot, hash256, WITNESS_RESERVED_VALUE} from "./utils.js";
...
export const validateBlock = (coinbase, txids) => {
    if (txids.length === 1) {
        throw new Error('Cannot validate empty block. You must include some transactions in the block to complete the challenge.');
    }

    const mempool = JSON.parse(readFileSync('./src/mempool.json'));
    ...
    if (coinbaseTx.outs.length !== 2) {
        throw new Error('Coinbase transaction must have exactly 2 outputs. One for the block reward and one for the witness commitment');
    }
    ...
    for (let i = 1; i < txids.length; i++) {
        const tx = JSON.parse(readFileSync(`./src/mempool/${txids[i]}.json`));
